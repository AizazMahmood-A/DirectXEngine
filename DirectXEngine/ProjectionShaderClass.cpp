...
bool ProjectionShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"../DirectXEngine/Projection.vs");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"../DirectXEngine/Projection.ps");
	if (error != 0)
	{
		return false;
	}
	...
	XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor, XMFLOAT3 lightPosition, float brightness)
{
	...
	// Lock the light position constant bufffer so it can be written to.
