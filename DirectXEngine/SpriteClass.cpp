...
void SpriteClass::Update(float frameTime)
{
	...
	if (m_frameTime >= m_cycleTime)
	{
		...
		SetRenderLocation(m_renderX + 1, m_renderY + 1);
	}
...
void SpriteClass::SetSize(int width, int height)
{
	m_bitmapWidth = width;
	m_bitmapWidth = height;
}

XMVECTOR SpriteClass::GetPositionVector()
{
	return XMVectorSet(m_renderX, m_renderY, 0, 1);
}