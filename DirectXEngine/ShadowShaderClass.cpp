...
bool ShadowShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
	ID3D11ShaderResourceView* texture, LightData* lightData, int lightDataSize)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, lightData, lightDataSize);
	if (!result)
	{
		return false;
	}
	...
	ID3D11ShaderResourceView* texture, LightData* lightData, int lightDataSize)
{
	...
	if (lightDataSize > 0)
	{
		...
		data->m_Light->GetViewMatrix(lightViewMatrix);
		data->m_Light->GetProjectionMatrix(lightProjectionMatrix);

		lightPosition = data->m_Light->GetPosition();
		diffuseColor = data->m_Light->GetDiffuseColor();
		ambientColor = data->m_Light->GetAmbientColor();
	...
	if (lightDataSize > 1)
	{
		LightData* data = &lightData[1];

		data->m_Light->GetViewMatrix(lightViewMatrix2);
		data->m_Light->GetProjectionMatrix(lightProjectionMatrix2);

		lightPosition2 = data->m_Light->GetPosition();
		diffuseColor2 = data->m_Light->GetDiffuseColor();
	...
	if (lightDataSize > 2)
	{
		LightData* data = &lightData[2];

		data->m_Light->GetViewMatrix(lightViewMatrix3);
		data->m_Light->GetProjectionMatrix(lightProjectionMatrix3);

		lightPosition3 = data->m_Light->GetPosition();
		diffuseColor3 = data->m_Light->GetDiffuseColor();
