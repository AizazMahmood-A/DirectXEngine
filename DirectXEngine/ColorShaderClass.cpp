...
bool ColorShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"../DirectXEngine/color.vs");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"../DirectXEngine/color.ps");
	if (error != 0)
	{
		return false;
	}
	...
	WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	D3D11_BUFFER_DESC matrixBufferDecs;
	...
	matrixBufferDecs.Usage = D3D11_USAGE_DYNAMIC;
	matrixBufferDecs.ByteWidth = sizeof(MatrixBufferType);
	matrixBufferDecs.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	matrixBufferDecs.CPUAccessFlags = D3D10_CPU_ACCESS_WRITE;
	matrixBufferDecs.MiscFlags = 0;
	matrixBufferDecs.StructureByteStride = 0;

	// Create the constant buffer pointer so we can access the vertex 
	// shader constant buffer from within this class.
	result = device->CreateBuffer(&matrixBufferDecs, NULL, &m_matrixBuffer);
	if (FAILED(result))
	{
		return false;
	}
