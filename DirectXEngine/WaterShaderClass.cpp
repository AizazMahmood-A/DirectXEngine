...
bool WaterShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"../DirectXEngine/Water.vs");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"../DirectXEngine/Water.ps");
	if (error != 0)
	{
		return false;
	}
	...
	ID3D11ShaderResourceView* normalTexture, float waterTranslation, float reflectRefractScale)
{
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, reflectionMatrix, reflectionTexture, refractionTexture,
...
bool WaterShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, XMMATRIX reflectionMatrix, ID3D11ShaderResourceView* reflectionTexture,
